...
local subs = {
    test_git = {
        ...
        run = function(_)
            require("csc").test_git_integration()
        end,
    },
    analyze = {
        desc = "Analyze repository scopes",
        run = function(_)
            require("csc").analyze_repository_scopes()
        end,
    },
    status = {
        desc = "Show status for current buffer",
        run = function(_)
            require("csc").show_commit_status()
        end,
    },
}
...
local function complete_sub(_, line)
    ...
    for name, _ in pairs(subs) do
        if name:find("^" .. vim.pesc(arg)) then
...
function M.setup(logger)
    ...
    vim.api.nvim_create_user_command("CSC", function(opts)

    ...
    local job_id = vim.fn.jobstart(cmd, {
        ...
        on_exit = function(_, exit_code)
            if exit_code == 0 then
                ...
                callback(
                    error_msg ~= "" and error_msg or "Git command failed",
                    nil
                )
            end
        end,
...
function M.get_git_log(opts, callback)
    ...
    local args = {
        ...
        "-n",
        tostring(opts.max_count),
    }

    M.run_git_command(args, {}, function(err, output)
        if err then
            callback(err, nil)
            return
        end
...
function M.test_git_commands(callback)
    ...
    M.run_git_command({ "--version" }, {}, function(err, output)

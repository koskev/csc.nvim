...
function M.show_commit_status()
    ...
    M.logger.log(table.concat(status_info, "\n"), vim.log.levels.INFO)
end
...
function M.setup(opts)
    ...
    local augroup =
        vim.api.nvim_create_augroup("CommitScopeBuffer", { clear = true })
    ...
    M.parser.get_scope_suggestions(
        ...
        function(err, suggestions)
            ...
            if #suggestions == 0 then
                M.logger.log(
                    "No scopes found in commit history",
                    vim.log.levels.WARN
                )
                return
            end
